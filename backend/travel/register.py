...
同时在此处完成旅游域的外部数据源接线（Phase 1 Provider 层）：真实路线数据源
（腾讯位置服务）必须在任何排程发生之前注入 estimate_leg，否则第一轮排程就会
用本地估算，行程单上的 source 会与实际不符。接线经 providers/travel 组装，
由 Provider 层负责时效标注与远期出行日期降级策略。
"""
