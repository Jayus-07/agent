...
base_url 优先级 = credentials.base_url > 环境变量 OLLAMA_BASE_URL > config 默认值。
"""
import os
...
def build_ollama(
    ...
    base_url 优先级：credentials > 环境变量 OLLAMA_BASE_URL > config 默认值。
...
def get_ollama_balance(credentials: ProviderCredentials | None = None) -> dict:
    """Ollama 本地部署，不消耗云端余额"""
