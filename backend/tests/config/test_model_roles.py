...
覆盖 docs/model-config-governance-design.md §3 的契约：
  ...
  - resolve_raw：字面值（DB 覆盖 → 代码默认），**不展开 inherit**
...
def test_raw_ignores_env(monkeypatch):
...
def test_effective_expands_inherit_when_empty(monkeypatch):
    _clear_role_env(monkeypatch)
    inject_overrides({"main": "qwen3.7-plus"})
...
def test_effective_does_not_expand_when_db_set(monkeypatch):
    _clear_role_env(monkeypatch)
    inject_overrides({"main": "qwen3.7-plus", "doc": "qwen2.5:3b"})
    eff = resolve_effective("doc")
    assert eff["value"] == "qwen2.5:3b"
    assert eff["source"] == SOURCE_DB
...
def test_secret_ignores_legacy_env(monkeypatch):
