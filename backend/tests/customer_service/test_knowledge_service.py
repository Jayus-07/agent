...
class TestCSKnowledgeService:
    ...
    @patch("backend.rag.pipeline.RAGPipeline")
    def test_high_confidence_returns_answer(self, mock_cls):
    ...
    @patch("backend.rag.pipeline.RAGPipeline")
    def test_mid_confidence_returns_cautious_with_suffix(self, mock_cls):
        svc, mock_pipeline = self._make_service_with_mock_pipeline(
            "可能是这样的。",
    ...
    @patch("backend.rag.pipeline.RAGPipeline")
    def test_no_evidence_returns_refuse(self, mock_cls):
        svc, mock_pipeline = self._make_service_with_mock_pipeline(
            "",
    ...
    @patch("backend.rag.pipeline.RAGPipeline")
    def test_low_confidence_returns_refuse(self, mock_cls):
        svc, mock_pipeline = self._make_service_with_mock_pipeline(
            "不确定。",
    ...
    @patch("backend.rag.pipeline.RAGPipeline")
    def test_default_kb_ids_when_empty(self, mock_cls):
        svc, mock_pipeline = self._make_service_with_mock_pipeline(
            "回答",
    ...
    @patch("backend.rag.pipeline.RAGPipeline")
    def test_pipeline_exception_returns_refuse_with_error(self, mock_cls):
        mock_pipeline = MagicMock()
    ...
    @patch("backend.rag.pipeline.RAGPipeline")
    def test_multiple_kb_ids_propagated(self, mock_cls):
        svc, mock_pipeline = self._make_service_with_mock_pipeline(
            "综合回答",
