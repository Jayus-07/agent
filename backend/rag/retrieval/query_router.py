...
# （贴近评测集语料：上限是多少 / 金额是多少 / 不合格率超过多少 / RPO/RTO / 金额分界点）
...
# 跨文档查询：分别/各自 + 疑问，或 A 与 B 的对比
# 对比分支不要求"的"紧邻区别词（"采购审批和报销审批的权限分级有什么不同"）
_CROSS_DOC_RE = re.compile(
    r"(?:分别|各自)[^。？！?！]{0,10}?(?:需要|是什么|如何|怎样)"
    r"|(?:和|与|跟)[^。？！?！]{0,20}?(?:有什么|的)?(?:区别|不同|差异)"
    r"|(?:两|三|几)种(?:方式|方法|类型|情况|物流|渠道)"
)
