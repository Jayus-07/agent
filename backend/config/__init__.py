...
# 邮件引擎（批次1 Skill 集成）：smtp = 传统 SMTP 发送（仅 email.send）；
# agently = QQ 邮箱 Agently Mail CLI（agently-cli，支持收/搜/读/监听）。
# 引擎只影响发送路径；search/read/watch 三个读能力仅 agently 引擎提供。
EMAIL_ENGINE = os.getenv("EMAIL_ENGINE", "smtp").strip().lower()
# agently-cli 二进制名（PATH 中查找）；watch 长轮询单次上限（秒）
AGENTLY_BIN = os.getenv("AGENTLY_BIN", "agently-cli")
AGENTLY_WATCH_MAX_SECONDS = int(os.getenv("AGENTLY_WATCH_MAX_SECONDS", "300"))
...
from backend.config.rag import (
    ...
    ENABLE_SIMULATED_QUESTIONS, QUESTION_GEN_MAX_CHUNKS,
    ENABLE_TABLE_DESCRIPTIONS, TABLE_DESC_MAX_ROWS,
    LEGAL_CLAUSES_PER_PARENT,
    EMBED_BATCH_SIZE,
    MAX_CHUNKS_PER_DOC,
    DEFAULT_KB_ID,
    VECTOR_PG_POOL_MIN,
...
__all__ = [
    ...
    "TOOL_SELECTOR_LLM_MAX_TOKENS",
    "TOOL_SELECTOR_MODEL", "FC_TOOL_SELECTION_ROLLOUT_PERCENT",
    "FC_TOOL_SELECTION_ALLOWLIST",
    "TOOL_SELECTOR_FAST_PATH_SCORE", "TOOL_SELECTOR_MAX_CANDIDATES",
    "EMBEDDING_MODEL_PATH",
    ...
    "MAX_CHUNKS_PER_DOC",
    "DEFAULT_KB_ID",
    "VECTOR_PG_POOL_MIN", "VECTOR_PG_POOL_MAX", "VECTOR_HNSW_EF_SEARCH",
    "RAG_DOC_CANDIDATE_K", "BM25_SEARCH_K", "BM25_CANDIDATE_K",
    "HYBRID_SEARCH_K", "RERANK_TOP_K",
    ...
    "EMAIL_ENGINE", "AGENTLY_BIN", "AGENTLY_WATCH_MAX_SECONDS",
    # customer service
    ...
    "CS_DOMAIN_PATTERNS", "CS_RULE_MIN_HITS",
    "CS_CONFIDENCE_ANSWER", "CS_CONFIDENCE_CAUTIOUS",
