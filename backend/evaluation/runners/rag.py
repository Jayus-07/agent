...
def _run_rag(cases: list[TestCase], **kwargs) -> list[EvalResult]:
    ...
    for case in cases:
        ...
        try:
            ...
            # === 答案生成（只要 Ollama 可达就尝试，不再限于 --live）===
            gate = gate_mode()
            ...
            if case.metadata.get("generation_eval") or _generated_answer:
                ...
                # 用生成回答计算指标（如有），否则回退到检索文本
                answer_for_eval = _generated_answer or " ".join(d.get("page_content", "") for d in details)
            ...
            if not kwargs.get("no_ragas", False):
                _deferred_ragas.append((len(results), case, eval_ctx, ragas_metrics))
            ...
            if should_reject and confidence in ("medium", "high"):
    ...
    if _deferred_ragas:
        logger.info(f"[RAGAS] 并行评估 {len(_deferred_ragas)} 个 case（workers=2）")
