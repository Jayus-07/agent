"""
capability_registry.py — **Capability** 注册表（派生视图）

📜 命名沿革：本模块原名 ``orchestration/tool_registry.py``，因与
``backend/tools/tool_registry.py``（**Tool** 注册表）同名异物、是新人/LLM
误判的重灾区，2026-09-17 改名为 ``capability_registry``。旧路径保留废弃
shim（deprecated re-export），新代码一律 import 新模块。

⚠️ 与 Tool 注册表的区分（读 import 路径，别看名字）::

    backend/orchestration/capability_registry.py ← 本模块：**Capability** 注册表
                                                    数据源是 Skill（不是 Tool）
    backend/tools/tool_registry.py               ← **Tool** 注册表（34 个 @tool）

本模块是被 Planner / Critique / tool_selector /
