...
class ResourceMonitor:
    ...
    def get_memory_info(self) -> Dict[str, float]:
        ...
        try:
            ...
            return {
                'rss_mb': memory.rss / (1024 * 1024),  # 实际使用的物理内存(MB)
                'vms_mb': memory.vms / (1024 * 1024),  # 虚拟内存(MB)
                'percent': self.process.memory_percent(),  # 进程内存占比
                'system_percent': psutil.virtual_memory().percent,  # 系统内存占比
    ...
    def get_cpu_info(self) -> Dict[str, float]:
        ...
        try:
            return {
                'process_percent': self.process.cpu_percent(interval=0.1),  # 进程CPU占比
                'system_percent': psutil.cpu_percent(interval=0.1),  # 系统CPU占比
                'cpu_count': psutil.cpu_count(),  # CPU核心数
                'load_avg': psutil.getloadavg() if hasattr(psutil, 'getloadavg') else None  # 负载均值
            }
        except Exception as e:
    ...
    def check_resources(self, memory_threshold: float = 0.85, cpu_threshold: float = 0.90) -> bool:
        ...
        try:
            ...
            if system_mem_percent > memory_threshold:
                warning_msg = f"⚠️ 系统内存使用过高: {system_mem_percent*100:.1f}% (阈值: {memory_threshold*100}%)"
                warnings.append(warning_msg)
                logger.warning(warning_msg)
                
            # 检查进程内存
            if process_mem_percent > memory_threshold * 0.8:  # 进程阈值为系统的80%
                warning_msg = f"⚠️ 进程内存使用过高: {process_mem_percent*100:.1f}%"
                warnings.append(warning_msg)
                logger.warning(warning_msg)
                
            # 检查CPU
            if cpu_percent > cpu_threshold:
                warning_msg = f"⚠️ CPU使用率过高: {cpu_percent*100:.1f}% (阈值: {cpu_threshold*100}%)"
                warnings.append(warning_msg)
                logger.warning(warning_msg)
