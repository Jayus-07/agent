...
from tools.keyword import extract_doc_keywords, extract_chunk_keywords
...
def _smart_truncate(text: str, max_length: int) -> str:
    ...
    for sep in separators:
        ...
        if pos > best_pos:
            best_pos = pos
    if best_pos != -1:
        return text[:best_pos + 1]  # 保留分隔符
    else:
        # 回退到空格或直接截断（确保不破坏UTF-8）
