...
export default function TokensPage() {
  ...
  const load = useCallback(async (d: number) => {
    setLoading(true);
    try {
      setData(await getTokensSummary(d));
    } catch (e) {
  ...
  const loadCalls = useCallback(async (d: number, page: number, model: string) => {
  ...
  // 明细面板首次展开 / 筛选或翻页变化时加载
  useEffect(() => {
    if (showCalls) loadCalls(days, callsPage, callsModel);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [showCalls, days, callsPage, callsModel]);
  ...
  const cards = [
    ...
    { icon: <Wallet size={16} />, label: "预估成本", value: formatCost(totals?.cost_usd), sub: "按模型单价估算" },
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          <div>
            ...
            <p className="text-xs text-text-muted mt-0.5">LLM Token 消耗与成本统计（每次调用逐条计量）</p>
          </div>
        ...
        <p className="mt-4 text-[11px] text-slate-400">
          成本按 <code className="font-mono">AVAILABLE_MODELS</code> 单价表估算，未登记单价的模型计为 $0；
