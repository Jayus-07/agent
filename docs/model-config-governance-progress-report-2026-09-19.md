...
| **已完成但未入库** | F 批（会话级模型切换）全部代码已落码、测试通过，**8 个前端文件 + 2 个后端文件**留在工作区 |
| **被阻塞** | P1a-2（凭据链路闭合）、四个端点注册、F 批收口、0017 迁移提交 —— **全部卡在同一处** |
| **未开工** | 管理端页面、P2 剩余端点、模型选型进 DB 的数据通道、批次 A 后半 |
| **本会话验证** | 后端新增/回归测试合计 **172 passed**；前端 **36 passed**（28 + 5 + 3）；两端 `tsc --noEmit` 零错误 |
| **部署状态** | ⚠️ **所有改动均未部署**。未重启任何服务、未执行迁移、未占用任何端口。线上行为与本次工作开始前**完全一致** |

**一句话**：后端治理骨架（角色注册表 → 凭据抽象 → DB 覆盖层 → 探测服务 → 只读端点）已经成型，但**真正让「管理端配了就能用」成立的那一环（P1a-2 凭据链路）还差最后一步**，而它和另外三件事全部堵在同一个并发冲突上。
...
| 2 | **P1a-2 凭据链路闭合** | `proxy.py` 含双方改动（我的 `set_request_model` 在 37–93 行，对方的 budget/reserve 在 340+ 行），且依赖 `budget.py` / `pricing.py` / `quota.py` 未提交 → 单独提交主干 import 失败 | 对方 `budget` 线落定 |
| 3 | **F 批前端收口** | 8 个前端文件与并发会话的未提交改动**同文件** | 对方前端线落定 |
| 4 | **0017 迁移提交** | `down_revision` 依赖未提交的 0016 | 0014/0015/0016 入库 |
| 5 | **`vllm` 分发分支修复** | 与 #2 同一文件同一函数（`proxy._build_llm_for`） | 同 #2 |
...
**是否由我直接改 `proxy.py` 的 `_build_llm_for`？**

- **改的收益**：凭据链路 + `vllm` 分支这两条一起收掉（结论 1 + 结论 2），BYOK 才真正能用，P2 四个后端项全部解锁。
- **风险**：同一文件并发写有 lost-update 风险（本项目实测过「两个 Edit 都报 success、后写覆盖先写」）。不过 `_build_llm_for`（208–240 行）**落在双方改动的空白区**（我的在 37–93，对方的在 340+），插入本身是干净的。
- **不改**：交接单留在 `docs/coordination/2026-09-19-llm-model-config-handoff.md` 等对方落定。
...
| `docs/model-config-admin-ui-design.md` | 实现级 UI 设计（17 节）+ §15.1–§15.5 实施状态 |
