...
⚠️ 编号口径：**批次号一律以《总交接与实施计划》§5.3 的 B0~B15 为准**。
   docs/未完成功能进度汇总-2026-09-16.md 里的 `B1/B2/B5` 是**另一套旧清单序号**，
   两者已有实际冲突（旧 B5＝全量门禁，§5.3 B5＝网关伪头剥离）→ 见到单独出现的「B几」
   先确认是哪一份，拿不准就问，不要自行猜测。
