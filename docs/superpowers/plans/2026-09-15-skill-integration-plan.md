...
| 批次 | 内容 | 状态 | 完成日期 | 备注 |
|---|---|---|---|---|
| 1 | Agently Mail 升级 EmailSkill | 🔄 代码完成 | 2026-09-15 | 代码+测试全绿（388 passed 子集复验）并已提交；剩部署验收：服务器 `npm i -g @tencent-qqmail/agently-cli` + OAuth + 真实收发一次 |
| 2 | market_research Workflow（证据管线） | ⬜ 未开始 | — | |
| 3 | 评估职责拆分 + decision_log | ⬜ 未开始 | — | |
