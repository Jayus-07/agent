...
| B4 切流 | 配置发布五步流程演练（构建/校验/分发/滚动/回滚）→ 前端 `AUTH_GATEWAY_URL` 切 APISIX（先 dev/test 后 prod）→ 观察错误率/认证拒绝/SSE 中断；SCG 8080 保留原样为回滚路径（回滚=环境变量还原） | 回滚演练成功；24h 观察零异常 | **已完成**（提交 43fc2a9 + tag `apisix-gateway-b3`；发布/回滚演练通过：tag 检出→重启→行为恢复 10.8s；切流实证：前端 3100 命中 APISIX 日志 +1，回滚态 8080 → APISIX 零命中；24h 观察自动化已挂每小时巡检→.workbuddy/b4_watch.log。环境事故：Docker 引擎中途崩溃重启，已恢复并补齐 auth 栈容器） |
