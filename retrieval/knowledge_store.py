...
class PgVectorKnowledgeStore(KnowledgeStore):
    """PostgreSQL + pgvector 实现的向量知识库（预留，本 PR 不实现）。
    ...
    迁移步骤 (后续 PR):
    1. 实现本类的所有抽象方法
    2. 修改 retrieval/knowledge_store.py 的工厂函数
    3. 数据迁移脚本: ChromaDB → pgvector
    4. 业务层和 Agent 零修改
    """

    def __init__(self, persist_directory: str, embedding_function: Any):
        raise NotImplementedError(
            "PgVectorKnowledgeStore 尚未实现。"
            "当前请使用 ChromaKnowledgeStore。"
        )

    @classmethod
    def from_documents(cls, documents, embedding, persist_directory):
        raise NotImplementedError("PgVectorKnowledgeStore 尚未实现")
